...
### Added
- Onboarded the `nsf` and `eeoc` tenant orgs — standard `model_backend_monitors` rollout (Bedrock + Azure OpenAI monitors, infra-health monitors, and the per-tenant dashboards). Both are sub-accounts under aigov (like doli): SSO profiles `aigov-nsf`/`aigov-eeoc`, but the DD secrets keep the standard `usai-<tenant>-shared-dd-{api,app}-key` naming (unlike doli's bare names). Wired the usual `aws.<tenant>`/`datadog.<tenant>` provider pairs in `tenants.tf`. Total enabled: 25.
